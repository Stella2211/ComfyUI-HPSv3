...
class BackendTests(unittest.TestCase):
    def setUp(self):
        ...
        self.models = Path(self.workspace.name) / "models" / "hpsv3"
    ...
    def tearDown(self):
        sys.modules.pop("hpsv3_test_backend_hpsv3", None)
    ...
    def make_complete_download(self, path):
        ...
        (path / "config.json").write_text(json.dumps({"model_type": "qwen2_vl", "quantization_config": {
    ...
    def test_download_success_publishes_valid_staging_and_reuses_it(self):
        ...
        with mock.patch.object(self.backend, "RUNTIME_PYTHON", runtime), mock.patch.object(self.backend.subprocess, "Popen", return_value=process) as popen:
            ...
            self.backend.HPSv3Model(self.backend.DEFAULT_MODEL_NAME)
    ...
    def test_resolve_model_rejects_escape_and_invalid_or_incomplete_models(self):
        ...
        with self.assertRaisesRegex(ValueError, "inside models/hpsv3"):
            self.backend.resolve_model("../outside")
        invalid = self.models / "invalid"
        invalid.mkdir()
        (invalid / "config.json").write_text(json.dumps({"model_type": "qwen2_vl", "quantization_config": {"load_in_4bit": True}}), encoding="utf-8")
    ...
    def test_resolve_rejects_malformed_custom_model_and_wrong_architecture(self):
        ...
        qwen3 = self.models / "qwen3"
        self.make_complete_download(qwen3)
        config = json.loads((qwen3 / "config.json").read_text(encoding="utf-8"))
        config["model_type"] = "qwen3_vl"
        (qwen3 / "config.json").write_text(json.dumps(config), encoding="utf-8")
        with self.assertRaisesRegex(ValueError, "Qwen2-VL"):
            self.backend.resolve_model("qwen3")
        config["model_type"] = "qwen2_vl"
        for quant in (None, [], "nf4"):
            config["quantization_config"] = quant
            (qwen3 / "config.json").write_text(json.dumps(config), encoding="utf-8")
            with self.subTest(quant=quant), self.assertRaisesRegex(ValueError, "bitsandbytes NF4"):
                self.backend.resolve_model("qwen3")

    def test_resolve_rejects_custom_missing_processor_or_shard(self):
        missing_processor = self.make_model("missing-processor")
        (missing_processor / "tokenizer.json").unlink()
        with self.assertRaisesRegex(ValueError, "incomplete"):
            self.backend.resolve_model("missing-processor")
        missing_shard = self.make_model("missing-shard")
        (missing_shard / "model.safetensors").unlink()
        (missing_shard / "model.safetensors.index.json").write_text(json.dumps({"weight_map": {"x": "missing.safetensors"}}), encoding="utf-8")
        with self.assertRaisesRegex(ValueError, "incomplete"):
            self.backend.resolve_model("missing-shard")
    ...
    def test_run_preserves_image_order_options_and_sets_offline_environment(self):
        ...
        with mock.patch.object(self.backend.subprocess, "Popen", side_effect=popen):
            result = self.backend.HPSv3Model("model")._run("score", [image_a, image_b], prompts=["a & $(echo injected); | < >", "b"])
    ...
    def test_run_scores_pairs_independently_and_forwards_caption_options(self):
        ...
        class Scorer:
            ...
            def score(self, images, prompts):
                calls.append(("score", images, prompts))
        ...
        quantized = types.ModuleType("evaluation.hpsv3_quantized")
        quantized.HPSv3QuantizedInferencer = Scorer
        sys.modules.update({"evaluation": evaluation, "evaluation.hpsv3_quantized": quantized})
        spec = importlib.util.spec_from_file_location("hpsv3_test_worker", ROOT / "worker_hpsv3.py")
        ...
        self.assertEqual(calls[0][1], {"merged_dir": "m", "processor_dir": "m", "device": "cuda", "local_files_only": True})
        self.assertEqual(calls[1:], [("score", ["i1"], ["p1"]), ("score", ["i2"], ["p2"])])
        self.assertEqual(worker.run({**request, "operation": "caption", "max_new_tokens": 64}), ["ok"])
        self.assertEqual(calls[-1], ("caption", ["i1", "i2"], 64))
        with self.assertRaises(ValueError):
            worker.run({**request, "images": ["i1"], "prompts": ["p1", "p2"]})
        for name in ("evaluation", "evaluation.hpsv3_quantized", "hpsv3_test_worker"):
            sys.modules.pop(name, None)

    def test_main_serializes_deduplicated_warnings_with_result(self):
        evaluation = types.ModuleType("evaluation")
        quantized = types.ModuleType("evaluation.hpsv3_quantized")
        quantized.HPSv3QuantizedInferencer = object
        sys.modules.update({"evaluation": evaluation, "evaluation.hpsv3_quantized": quantized})
        spec = importlib.util.spec_from_file_location("hpsv3_test_worker_main", ROOT / "worker_hpsv3.py")
        worker = importlib.util.module_from_spec(spec)
        sys.modules[spec.name] = worker
        spec.loader.exec_module(worker)
        ...
        for name in ("evaluation", "evaluation.hpsv3_quantized", "hpsv3_test_worker_main"):
            sys.modules.pop(name, None)
