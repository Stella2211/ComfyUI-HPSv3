...
class BackendTests(unittest.TestCase):
    def setUp(self):
        ...
        self.models = Path(self.tmp.name) / "models" / "hpsv3pp"
    ...
    def tearDown(self):
        for name in ("hpsv3_test_backend", "folder_paths", "comfy", "comfy.model_management"):
    ...
    def make_model(self, path=None):
        ...
        (path / "config.json").write_text(json.dumps({"model_type": "qwen3_vl", "quantization_config": {"quant_method": "bitsandbytes", "bnb_4bit_quant_type": "nf4", "load_in_4bit": True}}), encoding="utf-8")
    ...
    def test_inference_is_direct_and_never_downloads(self):
        ...
        with mock.patch.object(self.backend, "_run_inference", side_effect=fake):
            model = self.backend.HPSv3PPModel("model")
            self.assertEqual(model.score([image], ["prompt"]), [0.5])
        fake.assert_called_once()
        args, kwargs = fake.call_args
        self.assertEqual(args[:3], ("hpsv3pp", (self.models / "model").resolve(), "score"))
        self.assertIs(args[3][0], image)

    def test_model_resolution_rejects_traversal_and_incomplete_shards(self):
        self.make_model()
    ...
    def test_model_listing_ignores_hidden_staging_directories(self):
        ...
        self.assertEqual(self.backend.list_models(), [self.backend.DEFAULT_MODEL_NAME, "model"])
