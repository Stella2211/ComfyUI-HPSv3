...
sys.path.insert(0, str(Path(__file__).resolve().parent / "third_party" / "hpsv3-4bit" / "hpsv3pp" / "src"))
...
def run(request):
    ...
    if request["operation"] == "score":
        # HPSv3++ conditioning depends on the batch; score each pair independently.
        return [scorer.score([image], [prompt], iter_step=0.0)[0]
