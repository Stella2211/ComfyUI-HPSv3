...
def resolve_model(name, allow_download=False):
    ...
    for directory in folder_paths.get_folder_paths("hpsv3pp"):
        ...
        if (candidate / "config.json").is_file():
            ...
            if name == DEFAULT_MODEL_NAME and not _validate_download(candidate):
                raise ValueError("The default HPSv3++ model is incomplete; repair or remove it before downloading.")
            return candidate
