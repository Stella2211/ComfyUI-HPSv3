...
def list_models():
    models = {str(config.parent.relative_to(root)) for directory in folder_paths.get_folder_paths("hpsv3")
...
def _validate_download(path):
    try:
        ...
        if config.get("model_type") != "qwen2_vl" or not isinstance(quant, dict) or quant.get("quant_method") != "bitsandbytes" or quant.get("bnb_4bit_quant_type") != "nf4" or not quant.get("load_in_4bit", quant.get("_load_in_4bit", False)):
            return False
...
class HPSv3Model:
    ...
    def _run(self, operation, images, **options):
        ...
        model_management.throw_exception_if_processing_interrupted()
        return _run_inference("hpsv3", model_path, operation, images, **options)
